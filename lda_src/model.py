...
def ldamodel(output_path, num_tops=3):
    ...
    serialize_corpus = corpora.MmCorpus(os.path.join(output_path, 'SerializedCorpus.mm'))
    # serialize_dict = corpora.Dictionary.load(output_path + "/new_dictionary")
    serialize_dict = corpora.Dictionary.load(os.path.join(output_path, 'dictionary'))
    ...
    ldamodel.save(os.path.join(output_path, 'ldamodel'))
    return ldamodel
